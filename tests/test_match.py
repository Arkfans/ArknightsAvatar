...
def _base_result(
    ...
    if error is not None:
        return {"error": error}
    return {"avatar": avatar, "threshold": threshold, "box": box or [0, 0, 1, 1], "box_norm": [0.0, 0.0, 0.001, 0.001]}
...
def test_plan_rematch(tmp_path: Path):
    avatars_dir = tmp_path / "avatars"
    avatars_dir.mkdir()
    for name in ["char_003_kalts.png", "char_003_kalts_2.png", "char_007_closre.png", "char_011_talula.png"]:
        (avatars_dir / name).write_bytes(b"")
    classified = {
        "characters": {
    ...
    old_characters = {
        "avg_003_kalts_1": _old_char(["char_003_kalts.png"], {"b.png": _base_result("char_003_kalts.png", 0.85)}),
        "avg_007_closre_1": _old_char(["char_007_closre.png"], {"b.png": _base_result("char_007_closre.png", 0.95)}),
        "char_011_talula_1": _old_char(["char_011_talula.png"], {"b.png": _base_result("char_011_talula.png", 0.95)}),
    }
    ...
    # --character 过滤后只考虑该角色
    to_rematch, kept = plan_rematch(classified, old_characters, avatars_dir, 0.9, character="avg_007_closre_1")
    ...
    data = {
        ...
        "bases": {
            "b.png": {
                ...
                "offsets": {
                    "char_003_kalts.png": [
                        {"offset": 0, "size": [180, 180], "threshold": 0.93, "x": 1, "y": 2, "best": True}
                    ]
                },
            },
...
def test_cli_rejects_invalid_rematch_confidence(tmp_path: Path, capsys: pytest.CaptureFixture):
    code = main(["--classified", str(tmp_path / "missing.json"), "--rematch-confidence", "1.5"])
    assert code == 1
    assert "--rematch-confidence" in capsys.readouterr().err


def test_cli_incremental_rematch_on_candidate_update(tmp_path: Path, capsys: pytest.CaptureFixture):
    ...
    assert report1["characters"]["avg_003_kalts_1"]["bases"]["avg_003_kalts_1$1.png"]["threshold"] < 0.9
...
def test_cli_incremental_skips_when_confident_and_candidates_change(tmp_path: Path, capsys: pytest.CaptureFixture):
