...
def derive_box(coef: np.ndarray, face_pos: dict, head_pos: dict) -> tuple[list[int], list[float]]:
    """由 face/head 检测框推导正方形裁切框，返回 (整数框, 浮点框)。"""
    f, h = face_pos, head_pos
    feats = [
        f["x"] + f["w"] / 2.0, f["y"] + f["h"] / 2.0, f["w"], f["h"],
        h["x"] + h["w"] / 2.0, h["y"] + h["h"] / 2.0, h["w"], h["h"],
    ]
    cx, cy, s = (np.asarray(feats + [1.0]) @ coef).tolist()
    exact = [cx - s / 2.0, cy - s / 2.0, cx + s / 2.0, cy + s / 2.0]
    box = [int(round(v)) for v in exact]
    return box, exact


def detect_one(item: dict, device: str | None, coef07: np.ndarray, coef08: np.ndarray) -> None:
    from npcavatar import detect
    from npcavatar.detect_bases import detect_head_top1
...
def draw_image(row: dict, out_png: Path, font: ImageFont.ImageFont, min_conf: float) -> None:
    ...
    if row["face_pos"]:
        f = row["face_pos"]
        dr.rectangle([f["x"], f["y"], f["x"] + f["w"] - 1, f["y"] + f["h"] - 1],
                     outline=FACE_COLOR + (255,), width=2)
    if row["head_pos"]:
        hp = row["head_pos"]
        dr.rectangle([hp["x"], hp["y"], hp["x"] + hp["w"] - 1, hp["y"] + hp["h"] - 1],
                     outline=HEAD_COLOR + (255,), width=3)
    ...
    reason = {"no_avatar": "no avatar candidate",
              "no_avatar_unreported": "not in match report (NPC)"}.get(row["reason"], row["reason"])
...
def write_outputs(rows: list[dict], accepted: list[dict], out_dir: Path,
                  min_conf: float, device: str | None, pool_size: int,
                  n: int, seed: int) -> None:
    ...
    for row in accepted:
        ...
        if safe.lower().endswith(".png"):
            safe = safe[:-4]
        name = (f"{row['character']}__{safe}__"
    ...
    with (out_dir / "summary.csv").open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["character", "base", "reason", "face_confidence",
                         "head_confidence", "derived_box07", "derived_box08", "accepted"])
        for r in rows:
            writer.writerow([
                r["character"], r["base"], r["reason"],
                "" if r.get("face_confidence") is None else round(r["face_confidence"], 4),
                "" if r.get("head_confidence") is None else round(r["head_confidence"], 4),
                r.get("derived_box07", ""),
                r.get("derived_box08", ""),
...
def main(argv=None) -> None:
    ...
    for item in order:
        ...
        ok = (fc is not None and hc is not None
              and fc >= args.min_conf and hc >= args.min_conf)
        item["accepted"] = ok
        if ok:
    ...
    write_outputs(rows, accepted, out_dir, args.min_conf, device,
                  len(pool), args.n, args.seed)
