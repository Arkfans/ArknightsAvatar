...
def run_unpack(
    ...
    for category in categories:
        ...
        for ab_path in ab_paths:
            ...
            if not force and progress.get(rel) == source_sha and _meta_exists(unpacked_dir, category, ab_path):
                stats[category]["skipped"] += 1
                continue
            try:
                unpack_one(ab_path, unpacked_dir, category, rel, source_sha, parser_cls=parser_cls)
                progress[rel] = source_sha
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="npcavatar-unpack", description="Unpack AB resources into PNG + metadata.")
    parser.add_argument("--config", help="Path to config file")
    parser.add_argument("--raw-dir", default="data/raw", help="Input AB cache directory")
    parser.add_argument("--unpacked-dir", default="data/unpacked", help="Output directory")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        stats = run_unpack(Path(args.raw_dir), Path(args.unpacked_dir), categories, force=args.force)
